...
def _ensure_output_dir(path):
    # Default output dirs live next to the module. That is writable for the
    # CLI reference; a desktop-app install ships the module read-only in
    # site-packages, so creation must not fail at import time. The suite
    # always redirects CHART_DIR / CSV_DIR / HERE into the run directory
    # (suite.exports) before calling any output-producing function.
    try:
        os.makedirs(path, exist_ok=True)
    except OSError:
        pass

_ensure_output_dir(CHART_DIR)
_ensure_output_dir(CSV_DIR)
