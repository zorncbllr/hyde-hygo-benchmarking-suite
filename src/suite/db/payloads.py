...
Parsing one payload is hot on the export path (~17MB, ~2.5M floats per
blob), so ``read_payload``/``write_payload`` use orjson (~5x faster than
stdlib json for these blobs). The on-disk format is unchanged plain JSON,
fully interoperable in both directions with previously written payloads.
"""
...
def write_payload(path: Path, data: Any) -> None:
    ...
    blob = zstd.ZstdCompressor(level=7).compress(orjson.dumps(data))
...
def read_payload(path: Path) -> Any:
    ...
    return orjson.loads(zstd.ZstdDecompressor().decompress(blob))
