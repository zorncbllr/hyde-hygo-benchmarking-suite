...
class HyDEQub:
    """
    HyDEQub — Lean Qubit-Encoded Hybrid Differential Evolution Network.

    Phase 1 — Qubit DE with quantum tunneling (60 % budget)
      Qubit register θ[i] ∈ [0, π/2]^D.  Observation: x = lo + sin²(θ) · width.
      DE/current-to-best/1/bin mutation in θ-space replaces the three v4
      operators (interference, entanglement, collapse) in a single step.
      Stagnation triggers quantum tunneling (reflection through π/4).
    ...
    Why qubit encoding matters:
      For symmetric bounds the global optimum maps to θ = π/4 in every
      dimension (sin²(π/4) = 0.5 → midpoint of [lo, hi]).  Tunneling
      through π/4 systematically sends individuals toward/through this
      region — a structured exploration that random reinjection cannot
      replicate.  This is what achieves 100 % convergence on Rastrigin 25D.

    Budget (25D, 5000 evals, no DSM tax):
      Phase 1: 3000 evals → ~35 generations  (v4 got only ~13 due to DSM)
      Phase 2: 2000 evals → ~50 CMA-ES gens  (v4 got only ~12 per restart)
    """
    ...
    def _emit_progress(self, phase, gen, theta=None):
        ...
        if theta is not None and self.dim == 2:
            p = self._observe(np.asarray(theta, dtype=float))[:200]
    ...
    def _observe(self, theta):
        return self.lo + np.sin(theta) ** 2 * self.width

    # ── Initialisation: LHS in θ-space + farthest-point reorder ──────
    ...
    # ── DE generation: DE/current-to-best/1/bin in θ-space ───────────

    def _de_gen(self, theta, fitness):
        N, dim = theta.shape
        best_t = theta[int(np.argmin(fitness))]
        ...
        mutants = theta + F[:, None] * (best_t - theta) \
        ...
        children_x = self._observe(children_t)
    ...
    def _tunnel(self, theta, fitness):
        N   = len(fitness)
        n_t = max(1, N // 2)
        ...
        if n_ev > 0:
            fitness[worst[:n_ev]] = self._eval_pop(tunneled_x[:n_ev])
        return theta, fitness
        ...
        while used < budget and self.eval_count < self.max_evals and restart < 6:
            ...
            while used < budget and self.eval_count < self.max_evals:
                try:
                    ...
                    ev = np.maximum(ev, 1e-20); D = np.sqrt(ev)
